...
public class PlayScreen implements Screen {
    ...
    private static final int[] KEYS = {Keys.A,
                                       ...
                                       Keys.Y,
                                       Keys.Z };
    private static final float INPUT_TIME = 5;
    ...
    public void loadLevel(String filename) {
        ...
        String[] ps = inputTokens[3].split(",");
        presents = new int[ps.length];
        for(int i = 0; i < ps.length; i++) {
            presents[i] = Integer.parseInt(ps[i]);
        }
        ...
        for(int i = 1; i < lines.length; i++) {
            ...
            switch(tokens[0].charAt(0)) {
                ...
                case 'E':{
                    ...
                    String[] ss = tokens[4].split(",");
                    int[] is = new int[ss.length];
                    for(int j = 0; j < is.length; j++) {
                        is[j] = Integer.parseInt(ss[j]);
                    }
                    Exit e = new Exit(x, y, dir, is, this);
                    objects.add(e);
                    break;
                }
                case 'O':{
    ...
    public void update(float delta) {
        ...
        if(inputTime >= INPUT_TIME) {
            Product p = new Product(presents[pnr], new TextureRegion(game.assetMngr.get("p"+presents[pnr]+".png", Texture.class)));
            p.setX(inputX);
            p.setY(inputY);
            fallings.add(new FallingProduct(p, input.getY(), input));
            inputTime = 0;
            pnr = (pnr+1)%presents.length;
        }
    ...
    public void render(float delta) {
        ...
        font.draw(game.batch, points+"", 150, 280);
